...
const Oawards = ({ fun, ID }) => {
  ...
  const currentTabAwardTypes = awardTypesByTab[activeTab] || [];
  ...
  const isCurrentTabValid = () => {
    const currentAwardTypes = awardTypesByTab[activeTab] || [];
    // At least 1 award type is mandatory
    if (currentAwardTypes.length === 0) return false;

    // Each award type must have at least 1 row with complete data
    for (const type of currentAwardTypes) {
      if (type.rows.length === 0) return false;
      for (const row of type.rows) {
        if (!row.quantity || !row.givenTo) return false;
      }
